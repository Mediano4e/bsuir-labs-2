...
class Client(User):
    ...
    def create_account(self) -> bool:
        ...
        if not self._username_check():
            logger.warning("Пользователь с таким именем уже существует")
            return False
        ...
        cursor.execute(query, (self._username, self._password, "Пользователь", self._dob))
    ...
    def create_review(self, title: str, content: str):
        if not self._id:
            raise Exception("Невозможно добавить отзыв для неавторизованного пользователя")
    ...
    def get_all_reviews(self) -> list[dict]:
        if not self._id:
            raise Exception("Невозможно получить отзывы для неавторизованного пользователя")
